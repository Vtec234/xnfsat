...
// Checks the global invariant:
// - all unsat clauses are in the unsat queue/stack
// - the unsat queue/stack is well-formed
static void yals_check_global_invariant (Yals * yals) {
  ...
  for (cidx = 0; cidx < yals->nxclauses; cidx++) {
    U1 sat = *(yals->xparitydb.start + cidx);
    for (p = yals_xlits(yals, cidx); (lit = *p); p++) {
      sat ^= yals_val(yals, lit);
    }
    assert (yals_xorsat(yals, cidx) == sat);
    if (!sat) nunsat++;
    if (yals->unsat.usequeue) {
      if (sat) continue;
      Lnk * l = yals->lnk[cidx + yals->nclauses];
      assert (l);
      assert (l->cidx == cidx + yals->nclauses);
    } else {
      int pos = yals->pos[cidx + yals->nclauses];
      if (sat) assert (pos < 0);
      else {
        assert_valid_pos (pos);
        assert (PEEK (yals->unsat.stack, pos) == cidx + yals->nclauses);
      }
    }
  }
...
//          yals_rand_mod (yals, 100) >= yals->opts.unfairfreq.val) {
  ...
  if (cidx >= yals->nclauses) {
    assert_valid_xcidx(cidx - yals->nclauses);
    LOG ("picking %s out of %d", yals_pick_to_str (yals), nunsat);
    assert (!yals_xorsat(yals, cidx - yals->nclauses));
    LOGCIDX(cidx, "picked");
  } else {
    assert_valid_cidx (cidx);
    LOG ("picking %s out of %d", yals_pick_to_str (yals), nunsat);
...
static float yals_dynamic_weighted_break (Yals * yals, int lit) {
  float res = yals_weighted_break (yals, -lit) + yals_xweighted_break(yals, ABS(lit));
  //printf ("%d\n", res);
  //if (res/2 % 2 == 1) exit(0);
...
#ifndef NDEBUG
  unsigned or_clauses_wb = wb;
  int nOrClauses = (int)(p - occs);
#endif

  // NOTE: we assume that any given variable appears in one XOR clause at most once
  occs = yals_xoccs(yals, ABS(lit));
  for (p = occs; (occ = *p) >= 0; p++) {
    cidx = occ >> LENSHIFT;
    if (!yals_xorsat(yals, cidx)) continue;
    w = yals->xorweight;
    assert (0 < w && w <= yals->strat.weight);
    ACCU (wb, w);
    b++;
...
#ifndef NDEBUG
  assert(!yals->crit || wb - or_clauses_wb == yals_xweighted_break(yals, ABS(lit)));

  int nXorClauses = (int)(p - occs);

  LOG ("literal %d breaks %u clauses out of %d OR, %d XOR results in weighted break %u",
    lit, b, nOrClauses, nXorClauses, wb);
#endif
...
static float yals_determine_weighted_break (Yals * yals, int lit) {
  float res;
  //if (yals->crit)
...
static double
yals_compute_score_from_weighted_break (Yals * yals, float w) {
...
/*
  if (yals->strat.correct)
    s = (w < yals->exp.max.cb) ?
          PEEK (yals->exp.table.cb, (unsigned)w) : yals->exp.eps.cb;
  else
    s = (w < yals->exp.max.two) ?
          PEEK (yals->exp.table.two, (unsigned)w) : yals->exp.eps.two;

  assert (s);
*/
  double s_hi, s_lo;

  s_hi = (w < yals->exp.max.cb) ?
          PEEK (yals->exp.table.cb, (unsigned)w) : yals->exp.eps.cb;
...
static int yals_pick_literal (Yals * yals, int cidx) {
  ...
  // Gather candidates for flipping together with break(..) values
  if (cidx >= yals->nclauses) { // XOR clause
    lits = yals_xlits(yals, cidx - yals->nclauses);
    for (p = lits; (lit = *p); p++) {
      // The literal to be flipped is always assumed to be false in the assignment
      if (yals_val(yals, lit)) lit = -lit;
      w = yals_determine_weighted_break(yals, lit);

      LOG("literal %d weighted break %f", lit, w);
  ...
  if (zero) {
    ...
    const float * wbs = yals->breaks.start;
    ...
    for (i = 0; i < n; i++) {
      ...
      LOG ("literal %d weighted break %f score %g", lits[i], w, s);
      PUSH (yals->scores, s);
    }
...
#endif
    LOG ("picked literal %d weighted break %f score %g", lit, w, s);
...
static void yals_dequeue_stack (Yals * yals, int cidx) {
  ...
  if (didx >= yals->nclauses)
    assert_valid_xcidx(didx - yals->nclauses);
  else
...
static void yals_dequeue (Yals * yals, int cidx) {
  if (cidx >= yals->nclauses) { // XOR clause
    LOG ("dequeue XOR %d", cidx - yals->nclauses);
    assert_valid_xcidx(cidx - yals->nclauses);
  } else {
    LOG ("dequeue OR %d", cidx);
    assert_valid_cidx (cidx);
  }
...
static void yals_enqueue (Yals * yals, int cidx) {
  if (cidx >= yals->nclauses) { // XOR clause
    LOG ("enqueue XOR %d", cidx - yals->nclauses);
    assert_valid_xcidx(cidx - yals->nclauses);
  } else {
    LOG ("enqueue OR %d", cidx);
    assert_valid_cidx (cidx);
  }
...
static void yals_make_clauses_after_flipping_lit (Yals * yals, int lit) {
  ...
  LOG ("flipping %d has made %d OR clauses", lit, made);
#ifndef NYALSMEMS
  {
    int updated = p - occs;
    ADD (update, updated);
...
static void yals_break_clauses_after_flipping_lit (Yals * yals, int lit) {
  ...
  LOG ("flipping %d has broken %d OR clauses", lit, broken);
#ifndef NYALSMEMS
  {
    int updated = p - occs;
    ADD (update, updated);
...
static void yals_check_xweighted_breaks(Yals * yals) {
#ifndef NDEBUG
  if (!yals->opts.checking.val) return;
  if (!yals->crit) return;

  float d, f, w1, w2, xw;
  for (int lit = 1; lit < yals->nvars; lit++) {
    // printf("checking xweightedbreak for lit %d\n", lit);
    if (yals_val(yals, lit)) {
      d = yals_dynamic_weighted_break(yals, -lit);
      f = yals_compute_weighted_break(yals, -lit);
    } else {
      d = yals_dynamic_weighted_break(yals, lit);
      f = yals_compute_weighted_break(yals, lit);
    }
    // w1 = yals_weighted_break(yals, -lit);
    // w2 = yals_weighted_break(yals, lit);
    // xw = yals_xweighted_break(yals, lit);
    // printf("dynamic: %d, from scratch: %d\n", d, f);
    // printf("weightedbreak: %d %d, xweightedbreak: %d\n\n", w1, w2, xw);
    assert(d == f);
  }
#endif
}

static void yals_flip_xclauses_after_flipping_lit(Yals * yals, int lit) {
  const int * p, * occs, * q;
  int cidx, occ;
  int tmp_lit;
#if !defined(NDEBUG) || !defined(NYALSTATS)
  ...
  // NOTE: we assume that any given variable appears in one XOR clause at most once
  // Otherwise, we'd have to loop once to compute which clauses flipped and again
  // to compute made, broken
  occs = yals_xoccs (yals, ABS(lit));
  for (p = occs; (occ = *p) >= 0; p++) {
    cidx = occ >> LENSHIFT;
    if (yals_flipxorsat(yals, cidx)) {
      yals_dequeue(yals, yals->nclauses + cidx);
      LOGCIDX (yals->nclauses + cidx, "made");
      if (yals->crit) {
        for (q = yals_xlits(yals, cidx); (tmp_lit = *q); q++) {
          yals_inc_xweighted_break(yals, tmp_lit, yals->xorweight);
        }
      }
#if !defined(NDEBUG) || !defined(NYALSTATS)
      made++;
      ...
      yals_enqueue(yals, yals->nclauses + cidx);
      LOGCIDX (yals->nclauses + cidx, "broken");
      if (yals->crit) {
        for (q = yals_xlits(yals, cidx); (tmp_lit = *q); q++) {
          yals_dec_xweighted_break(yals, tmp_lit, yals->xorweight);
        }
      }
#if !defined(NDEBUG) || !defined(NYALSTATS)
      broken++;
  ...
  LOG ("flipping %d has made %d, broken %d XOR clauses", lit, made, broken);
#ifndef NYALSTATS
  yals->stats.made += made;
  yals->stats.broken += broken;
#endif
...
static void yals_update_minimum (Yals * yals) {
  ...
  yals_check_xweighted_breaks (yals);
...
static void yals_flip (Yals * yals) {
  ...
  yals_flip_xclauses_after_flipping_lit (yals, lit);
  yals_update_minimum (yals);
}

/*------------------------------------------------------------------------*/

// Preprocess the formula via unit propagation
static void yals_preprocess (Yals * yals) {
  int nvars = yals->nvars, lit, other, next, occ, w0, w1;
  int * p, * c, * q, oldnlits, newnlits, satisfied, nsat, xnsat, nstr;
  // For each literal l, watches[l] is a stack of clauses
  // in which l is watched
  STACK(int) * watches, * s;
  // The truth assignment to literals
  signed char * vals;
  ...
  // Initialize watched literals for OR clauses
  ...
  // Propagate units found in the initial formula
  for (next = 0; !yals->mt && next < COUNT (yals->trail); next++) {
    ...
    if (vals[lit] < 0) {
      ...
      // Process OR clauses in which the negation of `lit` is watched
      s = watches - lit;
  ...
  oldnlits = COUNT (yals->cdb) + COUNT(yals->xcdb);

  if (yals->mt) goto DONE;

  // Remove satisfied OR clauses and falsified literals from OR clauses
  nstr = nsat = 0;
  q = yals->cdb.start;
  ...
  // Remove satisfied XOR clauses or report conflict
  xnsat = 0;
  q = yals->xcdb.start;
  int cidx = 0;
  int ccidx = 0;
  for (c = q; c < yals->xcdb.top; c++) {
    satisfied = PEEK(yals->xparitydb, cidx);
    int hasBlank = 0;
    for (p = c; (lit = *p); p++) {
      if (!vals[lit]) {
        hasBlank = 1;
      ...
      satisfied ^= (vals[lit] == 1 ? 1 : 0);
    }
    // if all literals are assigned, XOR clause must be satisfied
    if (!hasBlank) {
      if (!satisfied) {
        yals->mt = 1;
        goto DONE;
      ...
      // copy XOR clauses
      for (p = c; (lit = *p); p++) {
        if (vals[lit] != 0) { // `lit` was assigned, flip parity if 1 and remove it from clause
          if (vals[lit] > 0) {
            POKE(yals->xparitydb, cidx, PEEK(yals->xparitydb, cidx) ^ 1);
          }
          nstr++;
          continue;
        }
      ...
      POKE(yals->xparitydb, ccidx, PEEK(yals->xparitydb, cidx));
      ccidx++;
      assert (q >= yals->xcdb.start + 1);
      assert (q[-1]);
      *q++ = 0;
    }
    c = p;
  ...
  yals->xcdb.top = q;
  FIT (yals->xcdb);
  yals->xparitydb.top = yals->xparitydb.start + ccidx;
  FIT (yals->xparitydb);
  assert(COUNT (yals->xparitydb) == cidx - xnsat);
  newnlits += COUNT(yals->xcdb);
  assert(newnlits <= oldnlits);

  yals_msg (yals, 1, "removed %d satisfied OR clauses", nsat);
  yals_msg (yals, 1, "removed %d satisfied XOR clauses", xnsat);
  yals_msg (yals, 1, "removed %d unit literal occurrences", nstr);
  yals_msg (yals, 1,
  ...
  if (!initial && yals->opts.best.val) {
    ...
    if (!yals->opts.cacheduni.val)  {
      ...
      for (i = 0; i < ncache; i++) {
        ...
        assert (min >= 0);
        PUSH (yals->cands, i);
        if (yals->opts.cachedinv.val)
            PUSH (yals->scores, 1.0/min);
        else
...
static float yals_len_to_weight (Yals * yals, int len) {
  // const int uni = yals->strat.uni;
  // const int weight = yals->strat.weight;
  const float weight2 = (float)yals->opts.weight2.val / 100.0;
  const float weight3 = (float)yals->opts.weight3.val / 100.0;
  const float weight4 = (float)yals->opts.weight4.val / 100.0;
  const float weight5 = (float)yals->opts.weight5.val / 100.0;
  const float weight6 = (float)yals->opts.weight6.val / 100.0;
  const float weight7 = (float)yals->opts.weight7.val / 100.0;
  const float weight8 = (float)yals->opts.weight8.val / 100.0;
  float w;
...
static void yals_update_sat_and_unsat (Yals * yals) {
  ...
  yals->xorweight = ((float)yals->opts.xorweight.val / 100.0f);
  if (yals->crit) {
    memset (yals->weightedbreak, 0, 2*yals->nvars*sizeof(float));
    memset (yals->xweightedbreak, 0, yals->nvars*sizeof(float));
  }
  for (cidx = 0; cidx < yals->nclauses; cidx++) {
    satcnt = 0;
    lits = yals_lits (yals, cidx);
    ...
    if (!satcnt) {
      ...
      yals_inc_weighted_break (yals, yals->crit[cidx], yals->weights[cappedlen]); // TODO avoid mem on yals->weights if uniform weights
  }

  for (cidx = 0; cidx < yals->nxclauses; cidx++) {
    lits = yals_xlits(yals, cidx);
    U1 sat = *(yals->xparitydb.start + cidx);
    for (p = lits; (lit = *p); p++) {
      sat ^= yals_val(yals, lit);
    }
    yals_setxorsat(yals, cidx, sat);
    if (!sat) {
      yals_enqueue (yals, yals->nclauses + cidx);
      LOGCIDX (yals->nclauses + cidx, "broken");
    } else if (yals->crit) {
      for (p = lits; (lit = *p); p++) {
        // In a satisfied XOR clause, every literal is critical
        yals_inc_xweighted_break(yals, lit, yals->xorweight);
      }
    }
  }
  yals_check_xweighted_breaks (yals);
...
// Precomputes exponential/power literal weighing functions
static void yals_init_weight_to_score_table (Yals * yals) {
  ...
  cb = (double)yals->opts.cb.val / 100.0;
  ...
  eps = 0;
  invcb = 1.0 / pow(cb, 0.01);
  score = start;
  for (i = 0; score && i < 100000; i++) {
...
// Computes positions of clauses in the database, literal occurrences in the formula,
// and some statistics
static void yals_connect (Yals * yals) {
  ...
  int * occsptr, occs, len, lits, nused, uniform;
  ...
  // Count binary, ternary, quaternary OR clauses
  // as well as find min/max OR clause lengths
  ...
  // Count XOR clauses
  // TODO(WN): also count XOR lengths here?
  int nxclauses = 0;
  for (p = yals->xcdb.start; p < yals->xcdb.top; p = q+1) {
    for (q = p; *q; q++) {}
  ...
  yals_msg (yals, 1,
    "found %d binary, %d ternary and %d large OR clauses",
    nbin, ntrn, nclauses - nbin - ntrn);

  yals_msg (yals, 1,
    "size of OR literal stack %d (%d for large clauses only)",
    (int) COUNT (yals->cdb),
    ((int) COUNT (yals->cdb)) - 3*nbin - 4*ntrn);

  yals_msg (yals, 1,
    "size of XOR literal stack %d",
    (int) COUNT (yals->xcdb));
  ...
  if ((INT_MAX >> LENSHIFT) < nclauses)
    yals_abort (yals,
      "maximum number of OR clauses %d exceeded",
      (INT_MAX >> LENSHIFT));

  if ((INT_MAX >> LENSHIFT) < nxclauses)
    yals_abort (yals,
      "maximum number of XOR clauses %d exceeded",
      (INT_MAX >> LENSHIFT));

  yals->nclauses = nclauses;
  yals->nxclauses = nxclauses;
  yals->nbin = nbin;
  yals->ntrn = ntrn;
  yals_msg (yals, 1, "connecting %d OR clauses, %d XOR clauses", nclauses, nxclauses);
  ...
  // Init yals->xlits
  NEWN (yals->xlits, nxclauses);
  lits = 0;
  for (cidx = 0; cidx < nxclauses; cidx++) {
    yals->xlits[cidx] = lits;
    while (PEEK (yals->xcdb, lits)) lits++;
    lits++;
  }
  assert (lits == COUNT (yals->xcdb));
  ...
  // Count literals in XOR clauses
  for (cidx = 0; cidx < yals->nxclauses; cidx++) {
    for (p = yals_xlits(yals, cidx); (idx = *p); p++) {
      xcount[idx]++;
    }
  }
  ...
  for (lit = 1; lit < nvars; lit++) {
    int sum = count[lit] + count[-lit] + xcount[lit];
    occs += sum + 3; // `sum` slots for the actual occurrences + 3 end-of-list markers
  ...
  NEWN (yals->refs, 3*nvars);

  occs = 0;
  for (lit = 1; lit < nvars; lit++) {
    n = count[lit];
    LOG ("literal %d occurs %d times in OR clauses", lit, n);
    ...
    *yals_refs (yals, -lit) = occs;
    ...
    yals->occs[occs++] = -1;
    n = xcount[lit];
    LOG ("variable %d occurs %d times in XOR clauses", lit, n);
    *yals_xrefs (yals, lit) = occs;
    occs += n;
    yals->occs[occs++] = -1;
  }
  ...
  // TODO(WN): again, do we care about XOR lengths?
  // maybe we prefer to break short XORs than long ones?
  if (minlen == maxlen) {
    yals_msg (yals, 1,
      "all %d OR clauses are of uniform length %d",
      yals->nclauses, maxlen);
  } else if (maxlen >= 0) {
    yals_msg (yals, 1,
      "average OR clause length %.2f (min %d, max %d)",
      yals->avglen, minlen, maxlen);
    yals_msg (yals, 2,
      "%d binary OR %.0f%%, %d ternary OR %.0f%% ",
      nbin, yals_pct (nbin, yals->nclauses),
      ntrn, yals_pct (ntrn, yals->nclauses));
    yals_msg (yals, 2,
      "%d quaterny OR %.0f%%, %d large OR clauses %.0f%%",
  ...
  // Init yals->occs by writing, for each clause in which it occurs,
  // a bitpacked (clause index, clause length) value into each literal's
  // occurrences array
  for (cidx = 0; cidx < yals->nclauses; cidx++) {
  ...
  // Init the XOR parts of yals->xoccs like above
  for (cidx = 0; cidx < yals->nxclauses; cidx++) {
    p = yals_xlits(yals, cidx);
    len = 0;
    while (len < MAXLEN && p[len]) len++;
    while ((lit = *p++)) {
      occsptr = yals_xrefs(yals, lit);
      occs = *occsptr;
      assert_valid_occs(occs);
      assert (!yals->occs[occs]);
      yals->occs[occs] = (cidx << LENSHIFT) | len;
  ...
  if (yals->unsat.usequeue) NEWN (yals->lnk, nclauses + nxclauses);
  else {
    NEWN (yals->pos, nclauses + nxclauses);
    for (cidx = 0; cidx < nclauses + nxclauses; cidx++) yals->pos[cidx] = -1;
  }
  ...
  NEWN (yals->xorsat, yals->nxclauses);
  ...
  if (yals->opts.crit.val) {
    ...
    NEWN (yals->xweightedbreak, nvars);
  } else
...
void yals_del (Yals * yals) {
  ...
  if (yals->unsat.usequeue) DELN (yals->lnk, yals->nclauses + yals->nxclauses);
  else DELN (yals->pos, yals->nclauses + yals->nxclauses);
  DELN (yals->lits, yals->nclauses);
  DELN (yals->xlits, yals->nxclauses);
  if (yals->crit) DELN (yals->crit, yals->nclauses);
  if (yals->weightedbreak) DELN (yals->weightedbreak, 2*yals->nvars);
  if (yals->xweightedbreak) DELN (yals->xweightedbreak, yals->nvars);
  ...
  DELN(yals->xorsat, yals->nxclauses);
  ...
  if (yals->refs) DELN (yals->refs, 3*yals->nvars);
...
static void yals_new_clause (Yals * yals) {
  ...
  if (len == 1) {
    // NOTE(WN): unit XOR clauses are the same as OR unit clauses
    lit = PEEK (yals->clause, 0);
    LOG ("found unit clause %d in original formula", lit);
    PUSH (yals->trail, lit);
    return; // We don't add unit clauses
  }
  ...
  if (yals->isXor) {
    ...
    PUSH(yals->xcdb, 0);
    PUSH(yals->xparitydb, yals->xorParity);
    LOGLITS (yals->xcdb.top - len - 1, "new (length %d, parity %d) XOR", len, yals->xorParity);
  } else {
    for (p = yals->clause.start; p < yals->clause.top; p++) {
      lit = *p;
      PUSH (yals->cdb, lit);
    ...
    PUSH (yals->cdb, 0);
    LOGLITS (yals->cdb.top - len - 1, "new (length %d) OR", len);
  }
}

static signed char yals_sign (int lit) { return (lit < 0) ? -1 : 1; }

void yals_begin_xor_clause (Yals * yals) {
  if (COUNT(yals->clause) != 0 || yals->isXor)
    yals_abort(yals, "New XOR clause began without ending previous clause");
  yals->isXor = 1;
  yals->xorParity = 0;
}

void yals_add (Yals * yals, int lit) {
  if (lit) { // Add literal to current clause
    if (yals->isXor) {
      int idx;
      if (lit == INT_MIN)
      ...
      if (lit < 0) yals->xorParity ^= 1;
      // TODO(WN): triviality conditions?
      return;
    }
  ...
  } else { // DIMACS 0, start new clause
    if (!yals->isXor) {
      const int * p;
      for (p = yals->clause.start; p < yals->clause.top; p++)
      ...
      // NOTE(LL): yals->mark may be empty for XOR clauses
...
int yals_sat (Yals * yals) {
  ...
  if (yals->opts.maxorigvar.val != 0) {
    assert(yals->opts.maxorigvar.val < yals->nvars);
  }
...
void yals_stats (Yals * yals) {
  ...
  if (yals->opts.maxorigvar.val != 0) {
    ...
    int idx;
    for (idx = 1; idx <= yals->opts.maxorigvar.val && idx < yals->nvars; idx++) {
      origFlips += yals->flips[idx];
    }
    for (idx = yals->opts.maxorigvar.val + 1; idx < yals->nvars; idx++) {
      auxFlips += yals->flips[idx];
    }

    yals_msg (yals, 0,
      "%lld original var flips, %lld aux var flips",
      origFlips, auxFlips);
  }
